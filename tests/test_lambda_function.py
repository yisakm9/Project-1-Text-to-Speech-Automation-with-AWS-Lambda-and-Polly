...
def aws_credentials():
    ...
    # CHANGED: Region is now us-east-1
    os.environ['AWS_DEFAULT_REGION'] = 'us-east-1'

@pytest.fixture(scope='function')
def mock_environment(aws_credentials):
    """Set up the mocked AWS environment with S3 and Polly."""
    # The AUDIO_BUCKET environment variable is required by the lambda function
    ...
    with mock_aws():
        ...
        # MODIFIED: For 'us-east-1', the CreateBucketConfiguration parameter must be removed.
...
def test_s3_trigger_success(mock_environment):
    ...
    assert body['message'] == "Audio generated from S3 upload"
...
def test_api_gateway_trigger_success(mock_environment):
    ...
    assert response['statusCode'] == 200
    body = json.loads(response['body'])
    assert body['message'] == "Audio generated from API"
    assert body['audio_file'] == f"s3://{audio_bucket}/{expected_audio_key}"
...
def test_api_gateway_missing_text_error(mock_environment):
    ...
    """
    # 1. CREATE EVENT: Build a bad request payload (missing "text")
    api_event = {
        "body": json.dumps({"note": "This is not the right key"})
    }
...
def test_s3_trigger_with_spaces_in_key(mock_environment):
    ...
    notes_bucket = 'test-voicevault-notes'
    audio_bucket = os.environ['AUDIO_BUCKET']
    # S3 event sends the key URL-encoded
    note_key_encoded = 'my+test+file.txt'
    note_key_decoded = 'my test file.txt'
    expected_audio_key = 'my test file.mp3'

    # 1. SETUP: Upload file with its name
        ...
        "Records": [
            {
                "s3": {
                    ...
                    "object": {
                        "key": note_key_encoded # This is how S3 sends it
                    }
                }
            }
        ]
